...
export const unitText = unit => unit === 'mcg' ? 'µg' : unit;
...
const pendingCeilings = {
  isofundine: 500, 'calcium-chlorure': 1000, defibrillation: 200,
  amoxicilline: 2000, 'amoxicilline-clavulanique': 2000, cefotaxime: 3000, ceftriaxone: 4000,
  amiodarone: 300, atropine: 3000, hydrocortisone: 100, lidocaine: 100, magnesium: 2000,
  triphosadenine: 12, etomidate: 20, 'ketamine-analgesie': 80, 'midazolam-iv': 5,
...
const containerUnknown = new Set(['bicarbonate-acr', 'calcium-chlorure', 'magnesium', 'propofol', 'suxamethonium', 'levetiracetam', 'ssh', 'cafeine', 'flumazenil', 'sugammadex', 'glucose10', 'naloxone', 'noradrenaline', 'bicarbonate-hyperk', 'isofundine']);
...
function standardDoses(record, preparations) {
  ...
  const period = infusion ? (m.periodMinutes === 60 ? '/h' : '/min') : antibiotic ? ' (période à confirmer)' : m.unit === 'J' ? '/choc' : '/dose';
  return dosageBranches(m).flatMap(branch => preparations.map(prep => {
    const condition = [branch.condition, ...(m.weightMix ? [prep.condition] : [])].join(' · ');
    ...
    } else if (prep.concentration !== null) {
...
function preparationRows(record, variants) {
  ...
  if (m.volumeKind === 'withdrawal' || !m.stock) return [{ condition: 'Préparation', text: record.protocol.dilution }];
...
function ceilingText(record) {
  ...
  if (record.id === 'suxamethonium') return 'Aucun plafond de dose, conformément à la décision locale.';
...
export function buildMedicationSheet(record) {
  ...
  const presentation = presentations[record.id] || record.sourceCells[0] || 'Ampoule/flacon, quantité et concentration à renseigner.';
  ...
  if (containerUnknown.has(record.id)) questions.push('Préciser le volume du contenant disponible ; la concentration seule est renseignée.');
  if (record.id.startsWith('midazolam-') || record.id.startsWith('morphine-')) questions.push('Confirmer la présentation de l’onglet Ampoules par rapport au stock effectivement embarqué.');
  ...
  if (record.id === 'morphine-ivc') questions.push('Le tableau applique 20 µg/kg/h dès 3 mois ; Pédiadol décrit ce débit de 3 mois à 5 ans. Confirmer le schéma local au-delà de 5 ans.');
  ...
  if (record.id === 'isofundine') questions.push('Confirmer le plafond de 500 mL par bolus figurant dans la formule du tableau.');
  if (record.id === 'sufentanil') questions.push('L’onglet Ampoules contient aussi 250 µg/10 mL : confirmer que la préparation présentée utilise bien 50 µg/10 mL.');
  ...
  return { presentation, doseRows, preparations, administration, questions: [...new Set(questions)], ceiling: ceilingText(record), pendingCeiling: pendingCeilings[record.id] ?? null, particulars: record.protocol.particulars };
}
